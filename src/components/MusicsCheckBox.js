...
const MusicsCheckBox = (props) => {
  ...
  const { musics, albumMusics } = props;
  let options = [];

  musics.forEach(music => {
      options.push(
        <div key={music.id} className="flex items-center p-2 rounded hover:bg-gray-100">
          <input type="checkbox" onClick={(e) => albumMusics(e)} value={music.id} className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 focus:ring-2" />
